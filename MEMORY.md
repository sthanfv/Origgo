...
- 126. **Hito 126: Panel — etapa 3: Cazador, Auditoría y Precios editables (fin del plan del panel)**:
    ...
    - **Cazador:** último contacto del teléfono (marca en Upstash cuando pide la lista negra, máx. 1 por minuto, sin gastar Firestore), última publicación y las 20 más recientes (`admin_cazador_log`, escrito por la ingesta), inmuebles visibles.
    - **Auditoría:** últimas 100 acciones con nombres en español y filtro por acción (solo lectura).
    - **API:** `GET /api/admin/cazador`, `GET /api/admin/auditoria`, `GET|PUT /api/admin/precios` (`lib/admin/operacion.js`). Teléfono: se abren desde "Más".
    - **Pruebas:** `admin_operacion` 4/4 (nueva: validación, precio del panel usado por la orden nueva y por la web, base sin documento, estado del cazador), `test_ledger_wompi`, `pagos_idempotentes` 4/4, `reconciliation_cron` 8/8, `admin_clientes_pagos` 5/5, `smoke_freemium_flow` 7/7, `leads_ingest` 9/9, `leads_pagination` 5/5, `support_blacklist` 9/9; `tsc` y build OK; capturas en teléfono.
...
- 121. **Hito 121: Animación del código fiel a la referencia y cierre de sesión por inactividad (estándar OWASP/NIST)**:
    ...
    - **Sesión (el servidor manda):** `lib/admin/sesion.js` guarda `ini` y `act` en el JWT: 15 min sin peticiones → `SESION_INACTIVA`; 8 h absolutas aunque haya actividad; renovación deslizante como máximo una vez por minuto (`renovarSesion`, llamada desde `exigirAdminCon2FA(req, res)`); cookie de sesión (sin Max-Age). Tokens anteriores sin `act` piden el código otra vez. `GET /api/admin/estado?latido=1` renueva.
    - **Navegador (`src/admin/sesion-inactividad.ts`):** actividad compartida entre pestañas (localStorage), latido cada 4 min si hay actividad, aviso "¿Sigues ahí?" con cuenta regresiva 2 min antes (hay que pulsar "Seguir conectado"), cierre en todas las pestañas (BroadcastChannel), marcas de tiempo (funciona con pestañas en segundo plano). Firebase con `browserSessionPersistence`: cerrar la pestaña cierra la sesión de Google del panel. La pantalla de entrada explica el motivo del cierre.
    - **Pruebas:** `admin_2fa` 15/15 (nuevas: inactividad, renovación sin extender las 8 h, límite absoluto, cookie de sesión), `admin_auth` 9/9; `tsc` y build OK; capturas con Playwright de cada fase.

- 120. **Hito 120: Retiro de anuncios (Habeas Data) con radicado, índice de búsqueda y pestaña Retiros en el panel**:
    - **Antes:** cualquiera retiraba cualquier anuncio al instante sin identificarse, el formulario siempre decía "retirado" aunque fallara, y se guardaba en claro el teléfono y la IP de quien pedía.
    - **Ahora (estándar notice & takedown + Ley 1581 art. 15):** el formulario (`src/components/FormularioRetiro.tsx`) exige un dato que identifique el anuncio (código/enlace de Origgo, enlace original, celular del anuncio, o ciudad + barrio/descripción) y los datos de quien pide (nombre, correo, relación, motivo, declaración). Cada solicitud recibe radicado `HD-AAAAMMDD-XXXX` y plazo de 15 días hábiles (`lib/retiros.js`, colección `solicitudes_retiro`). Coincidencia exacta → retiro preventivo inmediato; aproximada → candidatos para revisión. El panel (`src/admin/PanelRetiros.tsx`, `lib/admin/retiros.js`, acciones `retiros`/`buscar`/`reindexar` de `api/admin.js`) permite retirar o rechazar (revierte el preventivo) con auditoría.
    - **Búsqueda sin servicios de pago (`lib/indice-busqueda.js`, `lib/busqueda-inmuebles.js`):** la ingesta guarda `indice_busqueda` (palabras sin tildes), `enlace_huella` (SHA-256) y `telefono_huella` (índice ciego HMAC con clave derivada de la de leads). **Pendiente del propietario: pulsar "Reindexar búsqueda" una vez en el panel** para indexar los inmuebles anteriores.
    - **Otros:** la ficha técnica de cada anuncio muestra el "Código del inmueble"; textos legales (es/en) describen el proceso real; la lista negra ya no guarda teléfono ni IP; `db.removeBlacklistedLead`, `db.coleccion` y `array-contains` en la base en memoria.
    - **Pruebas (solo las del cambio):** `retiros_habeas_data` 8/8 (nueva), `support_blacklist` 9/9 (adaptada al nuevo contrato), `leads_ingest` 9/9; `tsc` y `npm run build` OK. Documentación: `docs/RETIRO_DE_ANUNCIOS.md`, `docs/PANEL_ADMIN.md`.
    - **Pendientes:** acuse de recibo por correo (Resend), enlace "¿Es tu inmueble?" en la ficha. **Hallazgo aparte:** la opción de soporte "Sincronizar pago" de `SupportModal.tsx` es simulada (un `setTimeout` que dice "saldo acreditado" sin consultar nada); debe conectarse al reclamo real o quitarse.
