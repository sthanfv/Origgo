...
function renderizarInterfaz(dataset) {
  ...
  if (elTitle && config.titulo_modulo) {
    let raw = config.titulo_modulo.toLowerCase().includes('terminal')
      ? 'Inmuebles en venta <span class="editorial-italic">directo</span> de sus dueños'
      : config.titulo_modulo;
    elTitle.innerHTML = raw.replace(/</g, "&lt;").replace(/>/g, "&gt;")
      .replace(/&lt;span class="editorial-italic"&gt;/gi, '<span class="editorial-italic">')
      .replace(/&lt;\/span&gt;/gi, '</span>');
  }
  if (elSubtitle) {
    elSubtitle.textContent = (config.subtitulo && !config.subtitulo.toLowerCase().includes('radar de captación'))
      ? config.subtitulo
      : 'Sin intermediarios ni comisiones de inmobiliaria. Oportunidades y rebajas de urgencia detectadas hoy en Colombia antes de que lleguen a las agencias.';
  }
  if (config.total_sectores_monitoreados) {
    const txt = `${config.total_sectores_monitoreados} Sectores Monitoreados en Tiempo Real`;
  ...
  if (countEl) countEl.textContent = `${leads.length} oportunidades directas`;
  const headingEl = document.getElementById("catalogHeading");
  if (headingEl) headingEl.textContent = "Inmuebles Directos en Vivo";
        ...
        <div class="card-body">
          ...
          <div class="card-bottom-row">
            <div class="pricing-column">
              ...
              ${item.precio_m2 ? `
                <div class="pricing-sub-row">
                  <span class="unit-rate-badge">${escaparHtml(item.precio_m2)}</span>
                  ${item.descuento_arbitraje > 0 ? `<span class="unit-rate-badge badge-arbitraje" title="Oportunidad de Arbitraje Cuantitativo">-${item.descuento_arbitraje}% vs Mediana</span>` : ''}
                  ${item.rebaja ? `<span class="unit-rate-badge badge-rebaja" title="Rebaja de precio confirmada">${escaparHtml(item.rebaja)}</span>` : ''}
                </div>
              ` : ''}
            </div>
