...
async function ejecutarValidacionCompleta() {
  ...
  try {
    execSync(`node --test "${path.join(ROOT_DIR, 'tests', 'leads_ingest.test.js')}"`, { stdio: 'pipe' });
    assert(true, 'Pruebas unitarias de ingesta segura de leads desde hardware pasadas al 100%');
  } catch (e) {
    assert(false, `Fallo en test de ingesta de leads: ${e.message}`);
  }

  try {
    execSync(`node --test "${path.join(ROOT_DIR, 'tests', 'pagos_idempotentes.test.js')}"`, { stdio: 'pipe' });
    assert(true, 'Pagos: acreditación única y fallo cerrado (503) verificados al 100%');
  } catch (e) {
  ...
  try {
    execSync(`node --test "${path.join(ROOT_DIR, 'tests', 'admin_resumen.test.js')}"`, { stdio: 'pipe' });
    assert(true, 'Resumen del panel (cifras reales y alertas) verificado al 100%');
  } catch (e) {
  ...
  try {
    execSync(`node --test "${path.join(ROOT_DIR, 'tests', 'admin_codigo_correo.test.js')}"`, { stdio: 'pipe' });
    assert(true, 'Panel: código por correo (un solo uso, 10 min, 5 intentos, sesión de 2 h) verificado al 100%');
  } catch (e) {
    assert(false, `Fallo en test del código por correo: ${e.message}`);
  }

  try {
    execSync(`node --test "${path.join(ROOT_DIR, 'tests', 'admin_operacion.test.js')}"`, { stdio: 'pipe' });
    assert(true, 'Panel: precios editables, configuración pública y estado del cazador verificados al 100%');
  } catch (e) {
    assert(false, `Fallo en test de operación del panel: ${e.message}`);
  }

  try {
    execSync(`node --test "${path.join(ROOT_DIR, 'tests', 'seguridad_auditoria.test.js')}"`, { stdio: 'pipe' });
    assert(true, 'Regresiones de la auditoría de seguridad (enlaces de un solo uso, CSP, cierre de sesión, URL seguras) al 100%');
  } catch (e) {
    assert(false, `Fallo en regresiones de la auditoría de seguridad: ${e.message}`);
  }
