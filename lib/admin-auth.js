 ...
 * ese token y se exigen dos candados independientes:
 *   - el correo (verificado) está en la lista blanca `ADMIN_EMAILS` del .env, y
 *   - la cuenta tiene el custom claim `admin: true` (se asigna con `node scripts/admin-rol.js`).
 * El segundo factor (TOTP) se exige aparte, en lib/admin/acceso.js.
 *
 ...
 * @returns {Promise<{uid: string, email: string, email_verified: boolean, admin: boolean}>}
...
async function verificarIdTokenFirebase(token, opciones = {}) {
  ...
  return {
    uid: datos.sub,
    email: datos.email || '',
    email_verified: datos.email_verified === true,
    // Custom claim de rol: Firebase lo incluye firmado dentro del ID token.
 ...
 * @throws {Error & {status: number}} 401 sin token o inválido, 403 sin permiso (correo fuera de la
 *   lista, sin verificar o sin custom claim `admin`), 503 sin config.
 */
async function verificarAdmin(req, opciones) {
  ...
  if (!decoded.email_verified || !permitidos.includes(email) || !decoded.admin) {
