...
function checkRateLimit(req, res, opciones = {}) {
  ...
  // 🛡️ Comprobación de baneo local en memoria volátil
  const banEntry = requestStore.get(`ban:${ip}`);
  if (banEntry && Date.now() < banEntry.resetTime) {
    if (res && typeof res.status === 'function') {
      res.status(403).json({
        ok: false,
        error: 'IP_BLOCKED',
        message: 'Acceso denegado temporalmente por política de seguridad perimetral.'
      });
    }
    return false;
  }
...
async function checkRateLimitAsync(req, res, opciones = {}) {
  ...
  try {
    // Pipeline atómico en Upstash: Verificar Ban, INCR y EXPIRE condicional NX
    ...
    // 🛡️ Auto-defensa Honeypot: Si la IP está en lista de baneo, bloquearla con 403 inmediatamente
    const estaBaneada = Array.isArray(resultados) && resultados[0] && resultados[0].result !== null;
    if (estaBaneada) {
      if (res && typeof res.status === 'function') {
        res.status(403).json({
          ok: false,
    ...
    const conteoActual = Array.isArray(resultados) && resultados[1] && typeof resultados[1].result === 'number'
      ? resultados[1].result
      : 1;
...
/**
 * Banea automáticamente una dirección IP en Upstash Redis (Autodefensa Honeypot activa).
 * @param {string} ip - Dirección IP
 * @param {number} [segundos=86400] - Tiempo de bloqueo en segundos (por defecto 24 horas)
 * @param {string} [motivo='HONEYPOT_ATTACK'] - Razón forense
 */
async function banearIp(ip, segundos = 86400, motivo = 'HONEYPOT_ATTACK') {
  if (!ip) return;
  const redisUrl = process.env.UPSTASH_REDIS_REST_URL;
  const redisToken = process.env.UPSTASH_REDIS_REST_TOKEN;

  // Respaldo en memoria volátil
  requestStore.set(`ban:${ip}`, { count: 999999, resetTime: Date.now() + segundos * 1000, motivo });

  if (!redisUrl || !redisToken || typeof fetch === 'undefined') return;

  try {
    await fetch(`${redisUrl}/setex/ratelimit:ban:${ip}/${segundos}/${encodeURIComponent(motivo)}`, {
      method: 'GET',
