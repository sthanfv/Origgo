...
const marcasMemoria = new Map();

/**
 * Guarda una marca pequeña (p. ej. último contacto del cazador) en Upstash, sin tocar Firestore.
 * Si Upstash no está configurado, queda solo en la memoria de la instancia.
 */
async function guardarMarca(clave, valor, ttlSeg = 7 * 24 * 3600) {
  marcasMemoria.set(clave, valor);
  await comandoRedis(['SET', `${PREFIJO}marca:${clave}`, JSON.stringify(valor), 'EX', String(ttlSeg)]);
}

async function leerMarca(clave) {
  const v = await leerRedis(`marca:${clave}`);
  return v !== undefined ? v : marcasMemoria.has(clave) ? marcasMemoria.get(clave) : null;
}
...
module.exports = { obtenerConCache, invalidarCache, esErrorDeCuota, limpiarCacheMemoria, guardarMarca, leerMarca };
