...
const { obtenerConCache } = require('../cache');
...
async function obtenerIdsListaNegra() {
  ...
  const { valor } = await obtenerConCache(CACHE_LISTA_NEGRA, 30 * 60, cargarListaNegra);
  return valor;
}
...
async function handler(req, res) {
  ...
  try {
    ...
    // completa en Firestore. Ahora se lee como máximo cada 30 minutos (o al registrar un retiro).
