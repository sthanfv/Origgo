...
const db = require('../db');
const { obtenerPrecios, productoPorCodigo, beneficio } = require('../precios');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'recover_token') {
      ...
      // Un enlace de recuperación sirve UNA sola vez (su nonce se quema de forma atómica).
    ...
    if (action === 'claim_reference' && reference) {
      ...
      if (order) {
        ...
        if (order.tipo === 'suscripcion_ciudad') {
          planData = { plan: 'city', city: order.ciudad, days: diasOrden };
        ...
        if (partes.length >= 3) {
          // Orden no encontrada: producto deducido por el código, con los precios del panel.
          const code = partes[2];
          const producto = productoPorCodigo(await obtenerPrecios(), code);
