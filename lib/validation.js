...
// Lista de dominios de correos temporales / desechables bloqueados (Anti-Sybil)
const DOMINIOS_CORREO_DESECHABLE = new Set([
  'tempmail.com', '10minutemail.com', 'guerrillamail.com', 'mailinator.com',
  'yopmail.com', 'trashmail.com', 'dispostable.com', 'getnada.com',
  'fakemailgenerator.com', 'throwawaymail.com', 'sharklasers.com',
  'temp-mail.org', 'crazymailing.com', 'generator.email'
]);

/**
 * Normaliza canónicamente una dirección de correo para prevenir cuentas duplicadas
 * mediante alias '+' o variantes con puntos en proveedores como Gmail.
 * @param {string} emailRaw
 * @returns {string}
 */
function normalizarEmail(emailRaw) {
  if (!emailRaw || typeof emailRaw !== 'string') return '';
  const limpio = emailRaw.trim().toLowerCase();
  const partes = limpio.split('@');
  if (partes.length !== 2) return limpio;

  let [local, dominio] = partes;
  // Descartar sufijos de alias '+' (ej: usuario+prueba@gmail.com -> usuario@gmail.com)
  local = local.split('+')[0];

  // En Gmail y Googlemail, los puntos en el local-part son ignorados por los servidores de Google
...
// Esquema estricto para dirección de correo electrónico con protección anti-desechables
const emailSchema = z.string({
  ...
  .refine(val => !/[<>"';\{\}]/.test(val), {
    message: 'El correo contiene caracteres especiales no permitidos.'
  })
  .refine(val => {
    const dominio = val.split('@')[1] || '';
    return !DOMINIOS_CORREO_DESECHABLE.has(dominio);
  }, {
    message: 'Por seguridad, utiliza un correo personal o corporativo (no se permiten correos temporales).'
  });
...
// Esquema para crédito de bienvenida freemium (Doble Opt-In)
const welcomeCreditSchema = z.object({
  celular: phoneSchema,
  email: emailSchema,
  deviceId: z.string().trim().max(128).optional().nullable(),
  leadId: z.string().trim().max(80).optional().nullable(),
  lang: z.enum(['es', 'en']).optional().default('es')
});

// Esquema para verificación y activación de bienvenida freemium
const welcomeVerifySchema = z.object({
  token: z.string().trim().min(8, 'El token de verificación es inválido.').max(128),
  lang: z.enum(['es', 'en']).optional().default('es')
});
