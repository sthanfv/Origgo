...
const welcomeCreditSchema = z.object({
  ...
  leadId: z.string().trim().min(3).max(64).optional().nullable(),
  lang: z.enum(['es', 'en']).optional().default('es')
}, {
  required_error: 'El cuerpo de la solicitud no puede estar vacío.'
...
// Esquema para registro de eventos de telemetría y embudo de conversión (Funnel CRO)
const funnelEventSchema = z.object({
  etapa: z.enum(['visita_landing', 'interes_inmueble', 'intento_conversion', 'conversion_exitosa'], {
    required_error: 'La etapa del embudo es obligatoria.',
    invalid_type_error: 'Etapa del embudo inválida.'
  }),
  leadId: z.string().trim().max(64).optional().nullable(),
  ciudad: z.string().trim().max(50).optional().nullable(),
  tipo: z.enum(['freemium', 'pago']).optional().nullable(),
  plan: z.string().trim().max(30).optional().nullable(),
  montoCop: z.number().nonnegative().optional().nullable(),
