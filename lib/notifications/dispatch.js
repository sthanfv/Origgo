...
let vapidIniciado = false;
function inicializarVapidSeguro() {
  if (vapidIniciado) return true;
  const pubKey = process.env.VAPID_PUBLIC_KEY;
  const privKey = process.env.VAPID_PRIVATE_KEY;
  const sub = process.env.VAPID_SUBJECT || 'mailto:contacto@origgo.online';

  if (pubKey && privKey) {
    try {
      webpush.setVapidDetails(sub, pubKey.trim(), privKey.trim());
      vapidIniciado = true;
      return true;
    } catch (errVapid) {
      console.error('Error al configurar VAPID en webpush:', errVapid.message);
      return false;
    }
  }
  return false;
...
module.exports = async function handler(req, res) {
  ...
  const internalSecret = requireEnv('INTERNAL_API_SECRET', {
    testFallback: 'origgo_internal_secret_d4e8f1a2c3b5'
  });
  ...
  if (!inicializarVapidSeguro()) {
    return res.status(503).json({ ok: false, error: 'VAPID_NOT_CONFIGURED', message: 'Faltan credenciales VAPID en el servidor o son inválidas.' });
  }
