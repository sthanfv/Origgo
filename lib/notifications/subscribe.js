...
const { registrarSuscripcion } = require('../push-subscriptions');

// Clave pública VAPID oficial de Origgo (Fallback público garantizado $0 coste)
const VAPID_PUBLIC_KEY_FALLBACK = 'BOxsLRo4U5zEtBAu31sM199CSbxzOLhoFqE7V7tHJcVZ-kKTDUS8_F08emQ8Swzc0tQ4WCB7NvtmFLsVjC9Y7eQ';
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      try {
        await checkRateLimitAsync(req, res, { prefix: 'vapid_pubkey', maxRequests: 60, windowMs: 60 * 1000 });
      } catch (_) {
        // En caso de incidencia transitoria de Redis, degradación suave
      }

      const rawKey = process.env.VAPID_PUBLIC_KEY || VAPID_PUBLIC_KEY_FALLBACK;
      const publicKey = String(rawKey || '').trim().replace(/^["']|["']$/g, '');
  ...
  console.error('Error global en subscribe.js:', globalError);
  return res.status(500).json({ ok: false, error: 'INTERNAL_ERROR', message: globalError.message });
}
};
