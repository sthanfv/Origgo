...
/**
 * Recupera usuarios potencialmente elegibles para el ciclo de retención
 * (usuarios con Plan Pro o usuarios con créditos <= 2).
 * 
 * @param {number} [limitCount=100] - Máximo número de usuarios a procesar
 * @returns {Promise<Array<Object>>} Lista de usuarios candidatos
 */
async function getActiveUsersForRetention(limitCount = 100) {
  if (!usersRef) return [];
  return await withRetry(async () => {
    const snapshot = typeof usersRef.limit === 'function'
      ? await usersRef.limit(limitCount).get()
      : await usersRef.get();
    ...
    for (const doc of snapshot.docs) {
      const user = typeof doc.data === 'function' ? doc.data() : doc;
      if (!user || !user.phone) continue;

      const tienePlan = Boolean(user.plan && user.plan !== 'free');
      const tieneSaldoBajo = Number(user.credits || 0) <= 2;

      if (tienePlan || tieneSaldoBajo) {
