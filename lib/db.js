...
function createMemoryCollection(collectionName) {
  ...
  return {
    ...
    /** Documento con id automático (como Firestore `add`). */
    async add(data) {
    ...
    where(field, op, expectedVal) {
      const obtenerFiltrados = (limite = Infinity) => {
        ...
        for (const [id, item] of store.entries()) {
          const coincide =
            (op === '==' && item[field] === expectedVal) ||
            (op === 'array-contains' && Array.isArray(item[field]) && item[field].includes(expectedVal));
          if (coincide) {
            results.push({ id: String(id), data: () => JSON.parse(JSON.stringify(item)) });
...
/**
 * Ajuste manual de una cuenta desde el panel (soporte al cliente): suma o resta créditos y/o
 * cambia el plan, en UNA transacción. No crea cuentas: el cliente debe existir.
 * @param {string} phone
 * @param {{ delta?: number, plan?: 'free'|'city'|'national', ciudad?: string|null, dias?: number }} cambios
 * @returns {Promise<Object>} Cuenta actualizada.
 */
async function ajustarCuentaAdmin(phone, { delta = 0, plan, ciudad = null, dias = 30 } = {}) {
  const normPhone = cleanPhone(phone);
  if (!normPhone) throw new Error('Número de teléfono inválido');
  return await withRetry(async () => {
    const userRef = usersRef.doc(normPhone);
    return await db.runTransaction(async (t) => {
      const doc = await t.get(userRef);
      if (!doc.exists) throw Object.assign(new Error('Cliente no encontrado'), { status: 404 });
      const cuenta = doc.data();
      cuenta.credits = Math.max(0, Number(cuenta.credits || 0) + Number(delta || 0));
      if (plan === 'free') {
        cuenta.plan = 'free';
        cuenta.planCity = null;
        cuenta.planExpiresAt = null;
      } else if (plan === 'city' || plan === 'national') {
        cuenta.plan = plan;
        cuenta.planCity = plan === 'city' ? ciudad || cuenta.planCity || null : null;
        cuenta.planExpiresAt = new Date(Date.now() + Math.max(1, Number(dias) || 30) * 86400000).toISOString();
      }
      cuenta.updatedAt = new Date().toISOString();
      t.set(userRef, cuenta);
      return cuenta;
    });
  }, 1);
}
...
/**
 * Marca un token como usado UNA sola vez (enlaces de recuperación, etc.). Atómico en Firestore
 * (`create` falla si ya existe). Devuelve true la primera vez y false si ya se había usado.
 * @param {string} clave Identificador único del token (p. ej. su nonce).
 */
async function marcarTokenUsado(clave) {
  const id = String(clave || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 120);
  if (!id) return false;
  if (modoMemoria || !db || typeof db.collection !== 'function' || !leadsRef || !leadsRef.firestore) {
    if (!memoryStore.tokens_usados) memoryStore.tokens_usados = new Map();
    if (memoryStore.tokens_usados.has(id)) return false;
    memoryStore.tokens_usados.set(id, { ts: Date.now() });
    return true;
  }
  return await withRetry(async () => {
    try {
      await db.collection('tokens_usados').doc(id).create({ ts: Date.now() });
      return true;
    } catch (e) {
...
async function addBlacklistedLead(leadId, data = {}) {
  ...
  // Minimización de datos (Ley 1581): no se guarda el teléfono ni la IP de quien pide el retiro;
  // la solicitud completa vive en `solicitudes_retiro` con su radicado.
  const registro = {
    leadId: idLimpio,
    reason: String(data.reason || data.razon || 'solicitud_propietario').slice(0, 100),
...
/**
 * Quita un inmueble de la lista negra (p. ej. una solicitud de retiro rechazada en el panel).
 * @param {string} leadId
 */
async function removeBlacklistedLead(leadId) {
  const id = String(leadId || '').trim();
  if (!id) return false;
  if (memoryStore.blacklisted_leads) memoryStore.blacklisted_leads.delete(id);
  if (modoMemoria || !blacklistedLeadsRef) {
    guardarAlmacenLocal();
    return true;
  }
  return withRetry(async () => {
    await blacklistedLeadsRef.doc(id).delete();
    return true;
  });
}
...
module.exports = {
  ...
  marcarTokenUsado,
  ajustarCuentaAdmin,
  get usersRef() { return usersRef; },
  get ordersRef() { return ordersRef; },
  get transactionsRef() { return transactionsRef; },
  unlockLead,
  ...
  /** Colección por nombre (Firestore en producción; memoria en pruebas sin credenciales). */
  coleccion(nombre) { return db.collection(nombre); },
