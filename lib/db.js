 ...
 * Si el número ya reclamó previamente su crédito de regalo, retorna un error de negocio.
 * 
 ...
 */
async function claimWelcomeCredit(phone, email) {
  const normPhone = cleanPhone(phone);
  ...
  return await withRetry(async () => {
    return await db.runTransaction(async (t) => {
      ...
      if (doc.exists) {
        ...
        // Usuario existía previamente pero no había consumido su regalo freemium
        ...
        // Nuevo usuario en la plataforma Origgo
        user = {
          phone: normPhone,
