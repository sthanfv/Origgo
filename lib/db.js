...
function esErrorCuotaFirestore(err) {
  ...
  const code = err.code;
  return code === 8 || msg.includes('RESOURCE_EXHAUSTED') || msg.includes('Quota exceeded') || details.includes('Quota exceeded');
}

/**
 * Ejecuta una operación asíncrona de base de datos con reintentos exponenciales y jitter aleatorio.
 * Ante agotamiento de cuota de Firestore, conmuta automáticamente al almacenamiento local en memoria.
 * 
 * @template T
  ...
  for (let intento = 1; intento <= maxIntentos; intento++) {
    try {
      ...
      if (esErrorCuotaFirestore(err)) {
        conmutarAFallbackEnMemoria('RESOURCE_EXHAUSTED');
