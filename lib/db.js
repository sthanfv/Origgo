...
function conmutarAFallbackEnMemoria(razon = 'cuota_agotada') {
  if (firestoreQuotaExhausted) return;
  firestoreQuotaExhausted = true;
  console.warn(`[db] Circuit Breaker: conmutando a almacenamiento local en memoria (${razon}).`);
...
if (!db) {
  conmutarAFallbackEnMemoria('sin_credenciales_firestore');
}
...
function esErrorCuotaFirestore(err) {
  ...
  const code = err.code;
  return code === 8 || code === 4 || msg.includes('RESOURCE_EXHAUSTED') || msg.includes('Quota exceeded') || details.includes('Quota exceeded') || msg.includes('DEADLINE_EXCEEDED');
}

function ejecutarConTimeout(promesa, ms = 3500, mensaje = 'DEADLINE_EXCEEDED') {
  let timer;
 ...
 * Ante agotamiento de cuota o cuelgues de red en Firestore, conmuta automáticamente al almacenamiento local en memoria.
 * 
 * @template T
  ...
  for (let intento = 1; intento <= maxIntentos; intento++) {
    try {
      if (!firestoreQuotaExhausted) {
        return await ejecutarConTimeout(operacion(), 3500);
      }
      return await operacion();
    } catch (err) {
      if (esErrorCuotaFirestore(err)) {
        conmutarAFallbackEnMemoria(err.message || 'RESOURCE_EXHAUSTED');
        // Reintentar inmediatamente sobre la colección en memoria
        return await operacion();
      }
...
async function addBlacklistedLead(leadId, data = {}) {
  ...
  return withRetry(async () => {
    if (blacklistedLeadsRef && !firestoreQuotaExhausted) {
...
async function getBlacklistedLeadIds() {
  ...
  if (firestoreQuotaExhausted || !blacklistedLeadsRef) {
    return memoryIds;
  }
  return withRetry(async () => {
    if (blacklistedLeadsRef && !firestoreQuotaExhausted) {
...
async function verifyAndBurnWelcomeToken(token) {
  ...
  // 4. Acreditar 1 crédito al usuario en base de datos
  const celular = tokenDoc.celular;
  const userActual = await getUserByPhone(celular);
  const nuevoSaldo = Math.max(1, (userActual?.credits || 0) + 1);

  const userActualizado = await addCredits(celular, 1, userActual?.pin || null);
  ...
  return {
    ...
    user: {
      phone: celular,
      credits: userActualizado?.credits || nuevoSaldo,
      email: tokenDoc.email,
