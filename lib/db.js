...
if (!db) {
  ...
  leadsRef = createMemoryCollection('leads');
...
async function getBlacklistedLeadIds() {
  return withRetry(async () => {
    if (blacklistedLeadsRef) {
      ...
      return Array.from(memoryStore.blacklisted_leads.keys());
    }
  });
}
...
async function isLeadBlacklisted(leadId) {
  ...
  return withRetry(async () => {
    if (blacklistedLeadsRef) {
      ...
      return memoryStore.blacklisted_leads.has(idLimpio);
    }
  });
}
