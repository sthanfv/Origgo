...
if (!db) {
  ...
  function createMemoryCollection(collectionName) {
    ...
    return {
      ...
      async get() {
        const docs = Array.from(store.keys()).map(k => ({
          id: k,
          exists: true,
      ...
      limit(n) {
        return {
          async get() {
            const keys = Array.from(store.keys()).slice(0, n);
...
/**
 * Obtiene usuarios candidatos para el ciclo de retención (Planes activos o usuarios con actividad).
 * En Firestore usa límite de seguridad y en memoria filtra el Map.
 * @param {number} [limite=100] Máximo de usuarios a evaluar por pasada (defecto: 100)
 * @returns {Promise<Array<object>>}
 */
async function getActiveUsersForRetention(limite = 100) {
  if (!usersRef) return [];
  return await withRetry(async () => {
    // Si estamos en entorno con fallback en memoria directo
    if (typeof usersRef.where !== 'function' && typeof memoryStore !== 'undefined' && memoryStore.users) {
      const todos = [];
      for (const [_, u] of memoryStore.users) {
        if (u && u.phone) todos.push(u);
        if (todos.length >= limite) break;
      }
      return todos;
    }

    // En Firestore / Store modular: consultamos usuarios con límite de seguridad
    try {
      const snap = typeof usersRef.limit === 'function'
        ? await usersRef.limit(limite).get()
        : await usersRef.get();

      const docs = [];
      if (snap && typeof snap.forEach === 'function') {
        snap.forEach(d => {
          if (d.exists !== false) docs.push(typeof d.data === 'function' ? d.data() : d);
        });
      ...
      console.warn('[db] Error consultando usuarios para retención:', e.message);
      return [];
    }
  });
}
