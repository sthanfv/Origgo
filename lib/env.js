...
/**
 * Exige una variable de entorno. En producción rechaza credenciales de prueba.
...
function requireEnv(name, options = {}) {
  ...
  if (process.env.NODE_ENV === 'production' && /test_|^prv_test_|^pub_test_/.test(value)) {
    throw new Error(`CONFIGURACION_INSEGURA: ${name} usa credenciales de prueba`);
  }
  return value;
}
