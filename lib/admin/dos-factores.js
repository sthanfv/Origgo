 ...
 *   POST /api/admin/codigo-correo → envía un código de 6 dígitos al correo del administrador
 *                                 (alternativa sin teléfono; ver lib/admin/codigo-correo.js).
 *                                 Se valida en /verificar con { codigo, metodo: 'correo' }.
 *
...
const codigoCorreo = require('./codigo-correo');
...
module.exports = async function handler(req, res) {
  ...
  if (accion === 'codigo-correo') {
    if (req.method !== 'POST') return res.status(405).json({ ok: false, error: 'Use POST.' });
    const permitido = await checkRateLimitAsync(req, res, {
      prefix: 'admin_2fa_correo',
      maxRequests: 3,
      windowMs: 15 * 60 * 1000,
    });
    if (!permitido) return;
    try {
      const r = await codigoCorreo.enviarCodigo(admin);
      await registrarAuditoria({ accion: 'codigo_correo_enviado', email: admin.email, detalle: { enviado: r.enviado } });
      if (!r.enviado) {
        return res.status(503).json({ ok: false, error: 'No se pudo enviar el correo. Usa un código de respaldo.' });
      }
      return res.status(200).json({ ok: true, destino: r.destino });
    } catch (e) {
      console.error('[admin:2fa] Error enviando código por correo:', e.message);
      return res.status(503).json({ ok: false, error: 'No se pudo enviar el código. Intenta de nuevo.' });
    }
  }
  ...
  if (accion === 'verificar') {
    ...
    const porCorreo = cuerpo.metodo === 'correo';
    let resultado = null;
    try {
      resultado = porCorreo ? await codigoCorreo.validarCodigoCorreo(admin.uid, codigo) : await validarCodigo(codigo);
    } catch (e) {
    ...
    if (resultado === 'totp' || resultado === 'respaldo' || resultado === 'correo') {
      // El código por correo es un factor más débil: la sesión dura como máximo 2 horas.
      const horas = resultado === 'correo' ? codigoCorreo.SESION_CORREO_H : undefined;
      res.setHeader('Set-Cookie', cabeceraCookie(crearSesion(admin.uid, horas)));
      if (resultado === 'correo') await codigoCorreo.avisarIngresoPorCorreo(admin, req).catch(() => {});
    ...
    const error =
      resultado === 'reutilizado'
        ...
        : resultado === 'vencido'
          ? 'El código venció o ya se usó. Pide uno nuevo.'
          : resultado === 'bloqueado'
