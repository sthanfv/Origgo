...
function crearSesion(uid, duracionH = DURACION_H, { ini, ahora = Date.now() } = {}) {
  ...
  return signJwt({ sub: uid, aud: AUDIENCIA, ini: inicio, act: ahora }, secretoSesion(), restanteDias);
}
...
function estadoSesion(req, uid, ahora = Date.now()) {
  ...
  // Límite absoluto explícito (además del vencimiento del JWT): 8 h desde el código.
  if (ahora - datos.ini > DURACION_H * 3600 * 1000) return { valida: false, motivo: 'ausente' };
...
function renovarSesion(res, datos, ahora = Date.now()) {
  ...
  res.setHeader('Set-Cookie', cabeceraCookie(crearSesion(datos.sub, DURACION_H, { ini: datos.ini, ahora })));
}
