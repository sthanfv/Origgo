 ...
 *   - Verificación reciente (`v2fa`): momento del último código. Las acciones peligrosas
 *     (borrar, precios, saldos, retiros) exigen un código de hace menos de 10 minutos
 *     ("modo sudo", como GitHub): robar una sesión abierta no basta para hacer daño.
 *
...
const VERIFICACION_RECIENTE_MS = 10 * 60 * 1000;
 ...
 */
function crearSesion(uid, duracionH = DURACION_H, { ini, ahora = Date.now(), v2fa } = {}) {
  const inicio = ini || ahora;
  const restanteDias = (inicio + duracionH * 3600 * 1000 - ahora) / 86400000;
  // Sin `ini` es una sesión nueva: el código se acaba de verificar.
  const verificado = v2fa || (ini ? inicio : ahora);
  return signJwt(
    { sub: uid, aud: AUDIENCIA, ini: inicio, act: ahora, dur: duracionH, v2fa: verificado },
    secretoSesion(),
    restanteDias
  );
}
...
function renovarSesion(res, datos, ahora = Date.now()) {
  ...
  res.setHeader(
    'Set-Cookie',
    cabeceraCookie(crearSesion(datos.sub, duracionH, { ini: datos.ini, ahora, v2fa: datos.v2fa || datos.ini }))
  );
}
...
/** ¿El último código se verificó hace menos de 10 minutos? */
function verificacionReciente(datos, ahora = Date.now()) {
  const v = typeof datos.v2fa === 'number' ? datos.v2fa : datos.ini;
  return ahora - v <= VERIFICACION_RECIENTE_MS;
}

module.exports = {
  COOKIE,
  INACTIVIDAD_MS,
  VERIFICACION_RECIENTE_MS,
  verificacionReciente,
  crearSesion,
  estadoSesion,
  sesionValida,
  renovarSesion,
