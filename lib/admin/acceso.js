...
const { estadoSesion, renovarSesion, verificacionReciente } = require('./sesion');
 ...
 * @param {{ reciente?: boolean }} [opciones] `reciente`: acción peligrosa, exige un código
 *   verificado hace menos de 10 min (si no, 403 con código REVERIFICAR).
 * @returns {Promise<{uid: string, email: string}>}
 * @throws {Error & {status: number, codigo?: string}}
 */
async function exigirAdminCon2FA(req, res, opciones = {}) {
  ...
  if (opciones.reciente && !verificacionReciente(estado.datos)) {
    const e = new Error('Por seguridad, confirma tu código para hacer esta acción.');
    e.status = 403;
    e.codigo = 'REVERIFICAR';
    throw e;
  }
